...
/**************************************************************************/
/*!
	@brief Select all target in field
*/
/**************************************************************************/

void Adafruit_PN532::selectAll(void) {
	uint8_t command_select[2];

    command_select[0] = PN532_COMMAND_INSELECT;
    command_select[1] = 0x00;

    sendCommandCheckAck(command_select, 2);
}
...
bool Adafruit_PN532::readPassiveTargetID(uint8_t cardbaudrate, uint8_t * uid, uint8_t * uidLength, uint16_t timeout) {
  ...
  *uidLength = pn532_packetbuffer[12];
  ...
  for (uint8_t i=0; i < pn532_packetbuffer[12]; i++)
  {
   ...
   uint8_t command_deselect[2];

    command_deselect[0] = PN532_COMMAND_INDESELECT;
    command_deselect[1] = pn532_packetbuffer[8];

    sendCommandCheckAck(command_deselect, 10);
